...
def setup():
    global cp1,cp2,cp3,cp4
    size(500,500)
    cp1 = ControlPoint(50,450)
    cp2 = ControlPoint(250,50)
    cp3 = ControlPoint(250,50)
    cp4 = ControlPoint(450,450)
...
def draw():
    ...
    cx = lerp(cp3.x, cp4.x, t)
    cy = lerp(cp3.y, cp4.y, t)
    dx = lerp(ax, bx, t)
    dy = lerp(ay, by, t)
    ex = lerp(bx, cx, t)
    ey = lerp(by, cy, t)
    ...
    # ベジェ曲線
    curve.append(ControlPoint(fx, fy))
    noFill()
    stroke(0,255,0)
    beginShape()
    for p in curve:
...
def mousePressed():
    ...
    dst4 = dist(cp4.x,cp4.y,mouseX,mouseY)
    cp1.drag = False
    cp2.drag = False
    cp3.drag = False
    cp4.drag = False
    if dst4<=5: cp4.drag = True
    elif dst3<=5: cp3.drag = True
